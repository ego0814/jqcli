...
fs = pd.read_parquet(OUT / "fscore.parquet", columns=["symbol", "rebalance_date", "f_score", "complete"])
ep = ep[ep["complete"] & ep["ep_value"].notna()][["symbol", "rebalance_date", "ep_value"]]
fs = fs[fs["complete"] & fs["f_score"].notna()][["symbol", "rebalance_date", "f_score"]]
...
m["rank_fs"] = m.groupby("rebalance_date")["f_score"].rank()
rows = []
for T, g in m.groupby("rebalance_date"):
    if len(g) < 20: continue
    c = g["rank_ep"].corr(g["rank_fs"])
...
print("  判断:", verdict)
c.to_parquet(OUT / "ep_fscore_corr.parquet", index=False)
print("  输出: ep_fscore_corr.parquet rows={}".format(len(c)))
