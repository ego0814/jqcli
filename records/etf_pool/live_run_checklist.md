...
- `--end` 默认今天；`--start` 默认 20160101（**保持完整窗口**，避免复权锚点位移）
- 跑完会打印「重叠 (code,date) N 个；close 与旧文件不同 M 个」与「净增 X 行」：
  - **M 应为 0**（无复权事件时新旧值应完全一致）
  - 若 M > 0，说明除权/折算或数据修订 → 必须用完整窗口（默认 `--start 20160101`）刷新，
    否则新旧标度混用会在拼接处产生假收益
- 实测（2026-09-26）：
  - `--end 20260918` → 46,247 → 46,247 行（净增 0），重叠 46,247 个、**不同 0 个**（幂等）
  - `--end 20260926` → 46,247 → **46,327 行（净增 80 = 4 交易日 × 20 只）**，
    max(date) 2026-09-18 → **2026-09-24**，重复行 0
  - 7 只池在 09-24 的 close 与 Tushare 不复权价**逐只完全一致**（最大绝对差 0.0）
- 注：09-25/26 为休市，故 `--end 20260926` 的正确落点是 **2026-09-24**。
